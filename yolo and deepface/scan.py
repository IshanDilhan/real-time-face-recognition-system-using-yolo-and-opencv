...
VIDEO_PATH = "students/videos/6/4.mp4"
...
def encode_student_faces():
    ...
    for student_folder in os.listdir(STUDENT_IMAGES_PATH):  # Iterate through student folders
        ...
        if os.path.isdir(student_path):  # Ensure it's a folder
            encodings = []
            for img_name in os.listdir(student_path):  # Loop through 5 images per student
    ...
    for r in results:
        for box in r.boxes:
            ...
            try:
                ...
                # Get embedding for detected face (Use array instead of path)
